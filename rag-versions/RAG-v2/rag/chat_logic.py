...
SIMILARITY_THRESHOLD = 0.65  # For vector search
GLUCOSE_LEVELS = {
    "HYPO": 70,
    "HIGH": 180,
    "CRITICAL": 250,
...
class ChatLogic:
    ...
    def _estimate_glucose_fallback(self, food_description: str) -> dict:
        """Fallback GI-based estimator for glucose."""
    ...
    def _format_summary_for_llm(self, stats: dict) -> str:
        ...
        try:
            ...
            date_formatted = "the most recent day"
    ...
    def process_chat(self, user_id: str, message: str, ts_str: str) -> dict:
        """
        Main method to drive the two-shot Gemini flow.
        """
        ...
        try:
            ...
            insight_prompt_segment = ""
            if context.get("insight_data") and router_response.get("intent", "").startswith("get_summary"):
                insight_prompt_segment = f"""
                Additionally, you MUST generate ONE short, actionable insight based on the user's recent data.
                Use the following data to find a pattern: {json.dumps(context.get("insight_data"), default=str)}
                The insight should be in multiple sentences, like "It seems meals with white rice cause a bigger spike for you than noodles."
                """
            ...
            **Your Goal:**
            Respond to the user's message thoughtfully. Incorporate any completed task results naturally.

            **Available Information (for context only):**
            - Glucose Prediction: {context.get('glucose_prediction')}
            - Activity Recommendation: {execution_results.get('activity_recommendation')}
            - First Aid Notice: {execution_results.get('first_aid')}
            - Daily/Weekly Summary: {formatted_summary}
            {insight_prompt_segment}
            
            **Response Guidelines:**
            1.  Prioritize the user's immediate question.
            2.  If `first_aid` text is available, it is CRITICAL. Start your response with it.
            3.  If `glucose_prediction` is available, you MUST include the `pred_30m` and `pred_60m` values in your response. Explain that these are the estimated blood sugar levels in 30 and 60 minutes.
            4.  If the user is asking about what they ate, check the history and tell them. If you don't know, say so.
            5.  Include `activity_recommendation` if available.
            6.  Provide the summary if requested.
            7.  Keep the tone supportive. Use phrases like "Kumusta ka?", "Naa ko para nimo".
            8.  DO NOT return JSON. Your response must be a single, natural-language string.
