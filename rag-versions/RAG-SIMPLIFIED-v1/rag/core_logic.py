...
class BuhaiCoreLogic:
    def __init__(self):
        ...
        try:
            ...
            self.llm = GeminiLLM()
            logger.info("Successfully loaded DataHandler, Retriever, and LLM.")
    ...
    def _generate_bisaya_output(self, rule_output, event_data, retrieved_context):
        ...
        templates = {
            ...
            """,
             "predicted_hyperglycemia": f"""
                Instruction: Warn the user about a high prediction, and give them a heads up about the situation
                ...
                Example output: "Pag-amping, Nanay/Tatay. Base sa atong data, posible nga musaka imong blood sugar tungod sa imong gikaon. Suwayi og inom og daghang tubig."
            """,
    ...
    def get_insights(self, user_id: str, event_type: str, event_data: dict):
        ...
        """
        if not all([self.data_handler, self.retriever, self.llm]):
        ...
        if event_type == 'meal' and 'description' in event_data:
            ...
            if predicted_glucose:
                logger.info(f"Estimated glucose based on historical meal data: {predicted_glucose:.2f} mg/dL")
            else:
                logger.info("No historical data found for this meal to make an estimation.")
