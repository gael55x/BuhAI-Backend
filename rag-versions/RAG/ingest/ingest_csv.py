...
if GEMINI_API_KEY:
    ...
    # The model name corresponds to the user's request for 'text-embedding-004'
    embedding_fn = GoogleGenerativeAIEmbeddings(model="models/embedding-001", google_api_key=GEMINI_API_KEY)
else:
    logger.warning("GEMINI_API_KEY not found. Using dummy sentence transformer embeddings.")
...
def get_chroma_client():
    ...
    # The LangChain wrapper handles creating the embeddings, so we pass the raw class
    collection = client.get_or_create_collection(
        name=CHROMA_COLLECTION,
        # metadata={"hnsw:space": "cosine"} # This is default and not needed with LangChain wrapper
    )
...
def ingest_meal_events(session: Session, collection, embedding_function, file_path: str):
...
def ingest_cgm_aggregates(session: Session, collection, embedding_function, file_path: str):
...
def main():
    ...
    chroma_collection, embedding_function = get_chroma_client()

    try:
        ingest_cgm_stream(db_session, os.path.join(CSV_DIR, 'cgm_stream.csv'))
        ingest_meal_events(db_session, chroma_collection, embedding_function, os.path.join(CSV_DIR, 'meal_events.csv'))
        ...
        ingest_cgm_aggregates(db_session, chroma_collection, embedding_function, os.path.join(CSV_DIR, 'cgm_aggregates.csv'))
